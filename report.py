            ...
            if row_type == constants.SECTION_ROW:
                ...
                if row_value:
                    if row_type == constants.REPORT_ROW:
                        ...
                        report[list(report.keys())[-1]][row_value[0]].append(row_topic)
                    elif row_type == constants.ADDITIONAL_REPORT_ROW:
                        report[list(report.keys())[-1]]["Comments"].append(
...
parser = argparse.ArgumentParser(
    description="Generate a report.", epilog="Happy Coding! :)"
)
parser.add_argument(
    "csv_file_path", metavar="CSV filepath", type=str, help="input csv file path"
)
parser.add_argument("-f", "--format", default="text", help="output format (text/json)")
